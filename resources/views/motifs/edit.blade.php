...
<div class="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow-md">
    <a href="{{ route('motifs.show', $motif) }}" class="text-blue-500 hover:text-blue-700">&lt;&lt; {{ __('retour') }}</a>
    <h1 class="text-3xl font-bold mb-6 text-gray-800">{{ __('Modifier un motif') }}</h1>
    <form action="{{ route('motifs.update', $motif) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        <div>
            <label for="nom" class="block text-lg font-medium text-gray-700">{{ __('Nom') }}</label>
        ...
        <div class="flex items-start">
            ...
            <div class="ml-3 text-lg">
                <label for="is_accessible_salarie" class="font-medium text-gray-700">{{ __('Accessible aux salariés') }}</label>
            </div>
